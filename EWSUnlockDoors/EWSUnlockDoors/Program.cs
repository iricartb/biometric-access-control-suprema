         ...
         try {
            if (args.Count() == 5) {
               ...
               if (Program.isValidArguments(Program.sIPAddress, Program.nPort, Program.nId1, Program.nId2, Program.nId3)) {
                  ...
                  if (!bError) {
                     Console.WriteLine("[ SUCCESS ] Comando de desbloqueo efectuado correctamente a la controladora EWS.");
                  }
                  else Console.WriteLine("[ ERROR ] Comando de desbloqueo no se ha efectuado correctamente a la controladora EWS."); 
