               ...
               if (Program.isValidArguments(Program.sIPAddress, Program.nPort, Program.nId1, Program.nId2, Program.nId3)) {
                  ...
                  for (int nDoor = 1; nDoor <= 2; nDoor++) { 
                     ...
                     if ((k == 6) && ((((int) oDataReceived[4]) == 255) || (((int) oDataReceived[4]) == 249))) {
                        ...
                        // CRC (2 Bytes)
                        oChecksum = new CRC16calc(InitialCrcValue.NonZero2);
                        oChecksumBytes = oChecksum.ComputeChecksumBytes(new Byte[] { oData[1], oData[2], oData[3], oData[4], oData[5], oData[6], oData[7] });
                        ...
                        oDataReceived = new byte[100];
                        k = oStream.Read(oDataReceived, 0, 100);
                  ...
                  if (!bError) {
                     Console.WriteLine("[ SUCCESS ] Comando de libre salida efectuado correctamente a la controladora EWS.");
                  }
                  else Console.WriteLine("[ ERROR ] Comando de libre salida no se ha efectuado correctamente a la controladora EWS."); 
